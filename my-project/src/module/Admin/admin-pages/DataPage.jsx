...
const DataPage = () => {
  ...
  const fetchData = async () => {
    ...
    setSuccess('');
    try {
      const response = await adminService.getDataPage(activeTab);
  ...
  const handleSave = async () => {
    ...
    try {
      if (data[activeTab]) {
        // Update
        const response = await adminService.updateDataPage(activeTab, form);
        if (response.data.success) {
        ...
        // Add
        const response = await adminService.createDataPage({ type: activeTab, ...form });
        if (response.data.success) {
