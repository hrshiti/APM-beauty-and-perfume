...
const Dashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const featured = await adminService.getProductsBySection('featured');
      const bestSellers = await adminService.getProductsBySection('bestsellers');
      const mostLoved = await adminService.getProductsBySection('mostloved');
      ...
      setStats({
        totalProducts: allProducts.data.products?.length || allProducts.data.length || 0,
        featuredProducts: featured.data.length || 0,
        bestSellers: bestSellers.data.length || 0,
        mostLoved: mostLoved.data.length || 0,
  ...
  const fetchSalesAnalytics = useCallback(async () => {
    try {
      const response = await adminService.getSalesAnalytics(selectedPeriod);
  ...
  const fetchStockSummary = async () => {
    try {
      const response = await adminService.getStockSummary();
  ...
  const fetchRevenueAnalytics = useCallback(async () => {
    try {
      const response = await adminService.getRevenueAnalytics(selectedPeriod);
