...
const Orders = () => {
  ...
  const handleCancellationRequest = async (orderId, action) => {
    try {
      ...
      await adminService.handleCancellationRequest(orderId, action, reasonForRejection);
  ...
  const confirmCODReceipt = async (orderId) => {
    ...
    try {
      ...
      const response = await adminService.confirmCODReceipt(orderId, confirmedAmount);
  ...
  const handleConfirmRevenue = async () => {
    ...
    try {
      ...
      await adminService.confirmCODReceipt(selectedOrderForRevenue._id, confirmedAmount);
          ...
          <div className="border-b pb-3">
            ...
            <div className="text-sm space-y-1">
              ...
              {order.paymentMethod === 'cod' && (
                <p className="text-blue-700 text-xs mt-2">💳 Full payment will be collected on delivery</p>
              )}
              {order.paymentMethod === 'online' && (
                <p className="text-green-700 text-xs mt-2">✅ Full payment completed online</p>
              )}
            </div>
          </div>
