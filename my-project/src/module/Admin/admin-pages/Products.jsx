...
const Products = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (selectedSection === 'all') {
        response = await adminService.getProducts();
      } else {
        response = await adminService.getProductsBySection(selectedSection);
      }
      ...
      try {
        ...
        let productsWithCategoryNames = (response.data.products || response.data || []).map(product => ({
          ...product,
          categoryName: categoryIdToName[product.category] || product.categoryName || product.category
        }));
  ...
  const handleSectionToggle = async (productId, section) => {
    try {
      ...
      await adminService.updateProductSections(productId, {
