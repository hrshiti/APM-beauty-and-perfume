// Mock admin service - no API calls, just returns mock data
import {
  mockProducts,
  mockOrders,
  mockCategories,
...
const adminService = {
  ...
  login: async (credentials) => {
    ...
    if (credentials.email && credentials.password) {
      ...
      return { data: { token, user: { email: credentials.email, name: 'Admin' } } };
    }
  ...
  registerAdmin: async (credentials) => {
    ...
    localStorage.setItem('token', token);
    localStorage.setItem('admin_logged_in', 'true');
    localStorage.setItem('admin_user', JSON.stringify({ email: credentials.email, name: credentials.username }));
    return { data: { token, admin: { email: credentials.email, name: credentials.username } } };
  },

  checkAdminStatus: async () => {
    await delay();
    return { data: { adminExists: false, message: 'No admin exists. Please register.' } };
  },
  ...
  getWishlistAnalytics: async () => {
    ...
    return {
      data: {
        ...
        totalProducts: mockWishlists.reduce((sum, w) => sum + w.products.length, 0)
      }
    };
  },
  ...
  verifyToken: async () => {
    ...
    if (token && adminUserData) {
      ...
      return {
        ...
        user: {
          ...mockAdminUser,
          username: user.username || mockAdminUser.username,
          email: user.email || mockAdminUser.email
        }
      };
    }
    return { valid: false };
  ...
  processRefund: async (orderId) => {
    ...
    if (order.refundStatus !== 'completed') {
      ...
      order.refundTransactionId = 'REF-' + Date.now();
    }
    
    // Also update in mockRefunds if exists
    let refund = mockRefunds.find(r => r.orderId === orderId);
    if (!refund) {
      refund = {
        _id: 'refund_' + orderId,
        orderId: orderId,
      ...
      refund.refundCompletedAt = new Date().toISOString();
    }
