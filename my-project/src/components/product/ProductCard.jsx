  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100">
      ...
      <div className="px-2 md:px-3 pt-2 md:pt-3 pb-0.5 md:pb-1">
        <p className="text-[10px] md:text-xs text-gray-600 font-medium uppercase">{product.categoryName || 'PRODUCT'}</p>
      </div>
      ...
      <div className="px-2 md:px-3 pb-2.5 md:pb-3">
        ...
        <div className="flex items-center gap-0.5 md:gap-1 mb-1.5 md:mb-2 flex-wrap">
          <div className="flex">
            {[...Array(5)].map((_, i) => (
              <svg 
                key={i} 
                className={`w-3 h-3 md:w-4 md:h-4 ${i < Math.floor(product.rating || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} 
                viewBox="0 0 20 20"
              >
          ...
          <span className="text-xs md:text-sm font-semibold text-gray-800">{product.rating || 4.5}</span>
          <span className="text-[10px] md:text-xs text-gray-600">({(product.reviews || 0) > 1000 ? `${((product.reviews || 0) / 1000).toFixed(1)}K` : product.reviews || 0} Reviews)</span>
        </div>
        ...
        <button 
          ...
          className="w-full bg-black text-white py-2 md:py-2.5 rounded-md font-semibold text-xs md:text-sm hover:bg-gray-800 transition-colors"
        >
          ADD TO CART
        </button>
      </div>
